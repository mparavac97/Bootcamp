...
namespace Bootcamp_WebAPI.Controllers
{
	public class MoviesController : ApiController
	{
		...
		public async Task<HttpResponseMessage> GetAsync()
		{
			...
			if (movieService.GetMoviesAsync() == null)
			{
			...
			else
			{
				...
				foreach (MovieEntity movie in await movieService.GetMoviesAsync())
				{
		...
		public async Task<HttpResponseMessage> GetAsync(Guid id)
		{
			...
			if (await movieService.GetMovieByIdAsync(id) == null)
			{
				return Request.CreateResponse(HttpStatusCode.NotFound);
			}
			else
			{
				MovieRest movieRest = new MovieRest
				{
					Name = (await movieService.GetMovieByIdAsync(id)).Name,
					ReleaseYear = (await movieService.GetMovieByIdAsync(id)).ReleaseYear
				};
		...
		[HttpPost]
		[Route("api/values")]
		public async Task<HttpResponseMessage> PostAsync([FromBody]MovieRest movieRest)
		{
			MovieService movieService = new MovieService();
		...
		[HttpPut]
		[Route("api/values/{id}")]
		public async Task<HttpResponseMessage> UpdateMovieDataAsync(Guid id, [FromBody]MovieRest movieRest)
		{
			MovieService movieService = new MovieService();
			...
			await movieService.UpdateMovieDataAsync(id, movie);
